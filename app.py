...
def process_lead_text(csv_file, parsed_leads, selected_people):
    ...
    # Fill Group/Remark/Lead Source/Project ONLY on client rows
    cols_to_fill = ['Group', 'Remark', 'Lead Source', 'Project Interested In']
    for col in cols_to_fill:
        if col in df_csv.columns:
            ffilled = df_csv[col].ffill()
            df_csv.loc[valid_rows_mask, col] = ffilled[valid_rows_mask]
    ...
    # Drop any non-client rows so trailing "Facebook / Saraswati" only rows are removed
...
def process_files(csv_file, excel_files, selected_people):
    ...
    # Drop any non-client rows so trailing right-side-only values don't remain
    df_csv = df_csv[valid_rows_mask].reset_index(drop=True)
